...
//Drop Down
export const DropdownComponent = ({ items, placeholder, CustomStyle, dropDownContainerStyle, open, value, setOpen, setValue }) => {
  return (
    <DropDownPicker
      open={open}
      value={value}
...
export const FlatListComponent = ({ data, renderItem, listStyle, itemStyle, emptyMessage }) => (
  <FlatList
      data={data}
      renderItem={({ item }) => (
          <View style={[styles.itemContainer, itemStyle]}>
              {renderItem ? renderItem(item) : <Text style={styles.itemText}>{item}</Text>}
          </View>
      )}
      keyExtractor={(item, index) => index.toString()}
      ListEmptyComponent={<Text style={styles.emptyText}>{emptyMessage || "No items available"}</Text>}
